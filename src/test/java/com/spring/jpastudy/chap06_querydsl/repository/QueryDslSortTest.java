...
class QueryDslSortTest {
    ...
    void pagingTest() {
        //given
        int pageNo = 1; //jpa 제로베이스라 pageNo * amount 하면 됨
        int amount = 2;
        ...
        List<Idol> pagedIdols = factory
                .selectFrom(idol)
                .orderBy(idol.age.desc())
                .offset((pageNo - 1)*amount)  // 1페이지 0 2페이지 2 3페이지 4....
                .limit(amount)
                .fetch();

        // 총 데이터 수 select count(*)
    ...
    void pagingWithJpaTest() {
        //given
        PageRequest pageInfo = PageRequest.of(0, 2);
    ...
    @Test
    @DisplayName("아이돌을 이름 기준으로 오름차순으로 정렬하여 조회하세요.")
    void findByIdolNameSortedTest() {
        //given

        //when
        List<Idol> sortedIdols = factory.selectFrom(idol).orderBy(idol.idolName.asc()).fetch();
    ...
    //2. 아이돌을 나이 기준으로 내림차순 정렬하고,
    // 페이지당 3명씩 페이징 처리하여 1번째 페이지의 아이돌을 조회하세요.
    @Test
    @DisplayName("2")
    void findNamePagingTest() {
        //given
        PageRequest pageInfo = PageRequest.of(0, 3);

        //when
        List<Idol> idolList = factory.selectFrom(idol).orderBy(idol.age.desc()).fetch();
        Page<Idol> idols = idolRepository.foundAllByPaging(pageInfo);
    ...
    //3. "아이브" 그룹의 아이돌을 이름 기준으로 오름차순 정렬하고,
